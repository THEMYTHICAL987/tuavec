...
async function fetchProductReviews(productId, page = 1, limit = 10) {
    try {
        return await apiRequest(`/products/${productId}/reviews?page=${page}&limit=${limit}`);
    } catch (error) {
...
async function fetchBrands() {
    try {
        const data = await apiRequest('/products/meta/brands');
        return data.brands || [];
    } catch (error) {
...
async function searchProducts(query, limit = 10) {
    try {
        const data = await apiRequest(`/products/search?q=${encodeURIComponent(query)}&limit=${limit}`);
